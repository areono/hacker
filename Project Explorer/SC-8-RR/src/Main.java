...
class Process {
    ...
    public void incExecTime()          {  var i = executionTime++;}
...
class Jobs
{
    ...
    public Jobs(int num) {  // 생성자
        ...
        arrivalTimes = new int [num]; // num개의 원소를 가지는 정수형 arrivalTimes[] 배열을 생성
        System.out.print("input "+num+" arrival times: ");// 적절한 입력용 메시지를 출력하고
        UI.nextInts(arrivalTimes);// UI.nextInts(...)를 호출하여 프로세스들의 도착시간을 입력 받아
        //     arrivalTimes[]에 저장

        serviceTimes = new int[num];// num개의 원소를 가지는 정수형 serviceTimes[] 배열을 생성
        System.out.print("input "+num+" service times: ");// 적절한 입력용 메시지를 출력하고
        UI.nextInts(serviceTimes);// UI.nextInts(...)를 호출하여 프로세스들의 서비스시간을 입력 받아
        //     serviceTimes[]에 저장
...
class SPN extends  Scheduler{
    ...
    public void schedule(){
        super.schedule();
        Process min = null;// 이 부분 생각 해내지 못했다.
        for (Process p: readyQueue){
            if(min == null || p.getServiceTime() < min.getServiceTime())
                min = p;
        }
        currentProcess = min;
...
class HRRN extends Scheduler {
    ...
    public void schedule(){
        super.schedule();
        Process max = null;//max가 null
        double maxHRRN = 0.0;

        for(Process p : readyQueue){
            double hrrn = (double) (p.getWaitingTime(currentTime) + p.getServiceTime()) /p.getServiceTime();
            if(hrrn > maxHRRN ){
                maxHRRN = hrrn;
                max = p;
            }
...
class SRT extends Scheduler{
    ...
    public void schedule(){
        super.schedule();
        Process min = null;
        double srt = Double.MAX_VALUE;
        for(Process p : readyQueue){
            double minSRT = p.getRemainingTime();
            if(minSRT < srt){
                srt = minSRT;
                min = p;
            }
...
class RR extends Scheduler {
    ...
    @Override
    public void schedule(){//.스케줄 만들기 생각이 좀 필요한듯?
        super.schedule();
        if(timeQuantumExausted()){
            readyQueue.add(readyQueue.poll());//.poll 은 queue 가 비어 있으면 null을 발새 시킨다.
        }
        currentProcess = readyQueue.peek();
        execTime = 0;
    }
    ...
    private boolean timeQuantumExausted() {
        if(currentProcess !=null && execTime == quantum)
            return true;
        else
            return false;
    }
